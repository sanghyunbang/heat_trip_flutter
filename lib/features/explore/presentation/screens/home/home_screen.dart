...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /// ExploreScreen과 동일한 규격으로 검색 쿼리 생성
  Map<String, String> _buildSearch({
    required String q,
    String? contentTypeId,
    String? emotionCategoryId,
    String? sort,
  }) {
    ...
    return {
      ...
      'page': '0',   // Explore 기본값과 일치
      'size': '20',  // Explore 기본값과 일치
            ...
            slivers: [
              ...
              SliverToBoxAdapter(
                child: _HeroWithSearchBar(
                  ...
                  // ✅ Explore와 동일한 기본 파라미터로 이동
                  onSubmitted: (q) => _goExplore(qp: _buildSearch(q: q)),
                  onTapHero: () => _goExplore(qp: _buildSearch(q: '추천')),
                ),
              ),
                  ...
                  return Padding(
                    ...
                    child: _BoardSectionView(
                      ...
                      onTapCard: (b) => _goExplore(
                        qp: {
                          // ✅ cat3가 있으면 커서 모드(추천/탐색)
                          if (b.cat3List.isNotEmpty) ...{
                            'themeId': b.themeId,
                            if (b.contentTypeId != null)
                              'contentTypeId': '${b.contentTypeId}',
                            'cat3': b.cat3List.join(','),
                          }
                          // ✅ cat3가 없으면 검색 모드 → Explore 규격으로 통일
                          else ..._buildSearch(
                            q: b.query ?? '',
                            contentTypeId: b.contentTypeId?.toString(),
                            // 필요 시 정렬을 추가하고 싶다면 여기에 예: sort: 'pop,desc'
                          ),
                        },
                      ),
                    ),
...
class _BoardCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final String cover = (data.preview.isNotEmpty)
        ? data.preview.first
        : 'https://picsum.photos/seed/heattrip/1200/800';
